                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped" width="50%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row">{{$i}}</th>
                                    <th scope="row">{{$answer->user->username}}</th>
                                    <th scope="row">{!!$answer->answer!!}</th>
                                    <th scope="row">{!!$answer->question->question!!}</th>
                                    <th scope="row">{{$answer->gambar}}</th>
                                    <th scope="row">{{$answer->created_at}}</th>
                                    <td>
                                        <div class=" row">
                                            <div class="col">
                                                <form action="{{route('answer.delete', ['id' => $answer->id])}}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
